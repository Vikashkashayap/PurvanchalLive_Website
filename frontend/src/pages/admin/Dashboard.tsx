...
// Function to strip HTML tags from text (optimized to avoid DOM manipulation)
const stripHtmlTags = (html: string): string => {
  if (!html) return '';
  // Use regex to remove HTML tags - more efficient than DOM manipulation
...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (isAuthenticated() && !loading) {
      fetchNews(selectedCategory, 1); // Reset to page 1 when category changes
      setCurrentPage(1);
    }
  }, [selectedCategory]);
  ...
  const fetchNews = async (category?: string, page: number = 1) => {
    try {
      ...
      const response = await newsAPI.getAll(categoryParam, page, itemsPerPage);

      // News is already sorted by creation date from backend (newest first)
      setNews(response.news);
      setCurrentPage(response.pagination.page);
      setTotalPages(response.pagination.pages);
      setTotalNews(response.pagination.total);
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="text-gray-600">Published (Page)</div>
          </div>
          <div className="bg-white rounded-lg shadow p-6">
            <div className="text-2xl font-bold text-orange-600">
              {news.filter(item => !item.isPublished).length}
            </div>
            <div className="text-gray-600">Draft (Page)</div>
          </div>
          <div className="bg-white rounded-lg shadow p-6">
          ...
          {totalPages > 1 && (
            <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
              <div className="flex-1 flex justify-between sm:hidden">
                <button
                  onClick={() => fetchNews(selectedCategory, currentPage - 1)}
                  ...
                  <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                    ...
                    {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                      ...
                      return (
                        <button
                          key={pageNum}
                          onClick={() => fetchNews(selectedCategory, pageNum)}
                          disabled={loading}
